...
- 사용자는 현재 개발 미리보기와 화면·동작이 모두 같아야 한다고 명확히 했다. 이 PC에 격리된 PocketBase 검증 DB를 준비해 기존 가명 이전 데이터 70,014행을 실제 가져오고 전체 값을 대조했다. 원본 데이터 공유·외부 운영 서버 배포는 별도이며, 아직 MISO와 실시간으로 연결하지 않았다.

## 이어서 수행할 순서

1. 현재 개발 미리보기의 동작을 migration/PARITY-CHECKLIST.md 기준으로 비교한다. 필수 플랫폼 문서와 컬렉션 원본 권한 규칙·인덱스를 지원되는 경로로 확인한다. 현재 Code/Database Workbench 접근은 가능하다.
