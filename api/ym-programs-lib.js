...
//   [260905 통합⑧] 담당자 칸은 마스터에 계정NO(managers.계정NO)로 저장 — 화면엔 이름으로, 저장은 번호로(명단에 없는 값은 그대로). 명단에 계정NO 열이 없으면 변환 없음.
...
// [260905 통합⑧] 담당자 명단의 계정NO ↔ 이름 (ym-acct-lib 과 같은 규칙 — 격리 스코프라 require 대신 여기 둔다)
...
var ACCT_NO_RE = /^\d{3,}$/;   // ym-acct-lib 과 같은 규칙
...
function list(app, col, withIndex) {
  ...
  var out = [], am = acctMaps(app, col);
  for (var i = 0; i < recs.length; i++) {
    var d = rowData(recs[i]);
    if (!nz(d["콘텐츠구분"])) continue;                 // 올해 운영 명단 = 콘텐츠구분이 있는 행
    var row = toView(d); row["담당자"] = acctName(am, row["담당자"]);   // [통합⑧] 계정NO → 이름
...
function create(app, col, body) {
  ...
  var patch = fromBody(body); if (Object.prototype.hasOwnProperty.call(patch, "담당자")) patch["담당자"] = acctNo(acctMaps(app, col), patch["담당자"]);   // [통합⑧] 이름 → 계정NO
  var pk = Object.keys(patch); for (var j = 0; j < pk.length; j++) d[pk[j]] = patch[pk[j]];
  ...
  if (dupRec) {                                         // 같은 ID 가 이미 마스터에 있으면(이력 행) 새 행 대신 그 행을 채운다(빈 값은 덮지 않음)
    ...
    if (nz(ex["구분"]) === "사업") return { error: "사업 행(사업NO) 과 겹치는 프로그램ID: " + want, status: 409 };   // [통합⑦] 사업 행은 프로그램이 아니다
...
function patch(app, col, rowIdx, body) {
  ...
  var d = rowData(rec); var p = fromBody(body); if (Object.prototype.hasOwnProperty.call(p, "담당자")) p["담당자"] = acctNo(acctMaps(app, col), p["담당자"]);   // [통합⑧] 이름 → 계정NO
