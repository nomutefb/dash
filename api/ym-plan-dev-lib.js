...
function toD8(v) { var s = nz(v), m = s.match(/^(\d{4})(?:[-./](\d{1,2})[-./](\d{1,2})|(\d{2})(\d{2}))$/); if (!m) return ""; var y = +m[1], mo = +(m[2] || m[4]), day = +(m[3] || m[5]), dt = new Date(Date.UTC(y, mo - 1, day)); if (dt.getUTCFullYear() !== y || dt.getUTCMonth() !== mo - 1 || dt.getUTCDate() !== day) return ""; return m[1] + (mo < 10 ? "0" : "") + mo + (day < 10 ? "0" : "") + day; }
...
function nextBizCode(app, col, year, letter) {
  ...
  return yy + "-" + letter + (mx + 1 < 10 ? "0" : "") + String(mx + 1);
}
function nextProgramId(app, col, d8) {
  ...
  return p + "_" + (mx + 1 < 10 ? "0" : "") + String(mx + 1);
}
...
function dailyIndex(app, col) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (nz(d["구분"]) === "회차발권") { if (!e.tk) e.tk = { "유료": 0, "초대": 0, n: 0, freePresent: false }; e.tk["유료"] += num(d["발권유료"]); e.tk["초대"] += num(d["발권초대"]); if (nz(d["발권초대"]) !== "") e.tk.freePresent = true; e.tk.n++; }
    else if (nz(d["누계총인원"]) || nz(d["누계유료"])) { var dt = nz(d["기준일자"]); if (!e.last || dt > e.last["기준일자"]) e.last = { "기준일자": dt, "유료": num(d["누계유료"]), "무료": num(d["누계무료"]), "총": nz(d["누계총인원"]) !== "" ? num(d["누계총인원"]) : (num(d["누계유료"]) + num(d["누계무료"])), "금액": num(d["누계금액"]) }; }
  }
  return idx;
}
function audience(d, di) {
  ...
  if (e && e.tk && e.tk.n) { var fr = e.tk.freePresent ? e.tk["초대"] : (e.last ? e.last["무료"] : 0); return { "유료": e.tk["유료"], "무료": fr, "출처": "대장 발권 " + e.tk.n + "회차" + ((!e.tk.freePresent && e.last) ? " + 일일 무료" : "") }; } // 대장 행에 초대가 비어 있으면 일일 누계무료로 보충
...
function bizSums(progs) {
  ...
  for (var i = 0; i < progs.length; i++) { var d = progs[i]; if (status(d) === "취소") continue; s["프로그램수"]++; s["정산서매출"] += num(d["표시_수입"]); if (!nz(d["정산일"])) continue; s["정산수"]++; s["전표실적"] += progCost(d);  s["판매수수료"] += num(d["판매수수료"]); s["유료인원"] += num(d["발권유료"]); s["초대인원"] += num(d["발권초대"]); s["횟수"] += num(d["회차수"]); s["문화나눔"] += num(d["문화나눔"]); } // 정산된 프로그램만 합산(중간 정산) — 안 된 건 원장 값 유지
  return s;
}
function bizView(d, progs, m, di) {
  ...
  return { "사업코드": nz(d["사업코드"]) || nz(d["프로그램ID"]), "연도": yearOf(d), "분야": fieldLabel(d), "사업명": nz(d["정본명"]), "회계구분": nz(d["회계구분"]), "예산": nz(d["예산"]), "사업형태": nz(d["사업형태"]) || "미정", "공연수입": nz(d["공연수입"]) || "유료",
    ...
    "실지출": nz(d["전표실적"]), "수입": nz(d["정산서매출"]), "판매수수료": nz(d["판매수수료"]), "유료관객": nz(d["유료인원"]), "무료관객": nz(d["초대인원"]), "누적관객": String(num(d["유료인원"]) + num(d["초대인원"])), "문화나눔": String(s["문화나눔"]), "합계출처": s["정산수"] ? ("정산 " + s["정산수"] + "/" + s["프로그램수"] + "건 합" + (s["정산수"] < s["프로그램수"] ? " · 중간 합계" : " · 전체 정산")) : "사업비 원장", // 사업현황 표와 같은 칸(운영_사업비 = 마스터 사업 행)
...
function bizSave(app, col, body) {
  ...
  require(__hooks+"/ym-ticketlink-lib.js").refreshRevenue(app,col,{businessCodes:[code]});
  progs = programsOfBiz(app, col, code);
...
function inherit(pd, bd) {
  ...
  set("공동기획_기관", nz(bd["공동기획_기관"])); set("공동기획여부", nz(bd["공동기획_기관"])?"1":"0");
...
function programSave(app, col, body) {
  ...
  var tbd = body["날짜미정"] === undefined ? !!(R && nz(R.d["날짜미정"]) === "Y") : body["날짜미정"] === true || body["날짜미정"] === "Y";
  if (!s8) return { error: "시작일 필요(미정이면 그 달 1일 + 날짜미정 체크)", status: 400 };
  if (nz(body["종료일"]) && !e8) return { error: "종료일 형식 또는 날짜가 올바르지 않습니다", status: 400 };
  if (!e8) e8 = s8; if (e8 < s8) return { error: "종료일은 시작일보다 빠를 수 없습니다", status: 400 };
  ...
  var d = R ? R.d : {}, isNew = !R, oldBizCode = R ? nz(R.d["사업코드"]) : "";
  ...
  recomputeBiz(app, col, code); if (oldBizCode && oldBizCode !== code) recomputeBiz(app, col, oldBizCode);
  touchLastmod(app, col);
...
function recomputeBiz(app, col, code) {
  ...
  if (!s["정산수"]) {
    if (B.d._ymPlanLedger && B.d._ymPlanAggregate) { for (var restore = 0; restore < SUM_MAP.length; restore++) { var rk = SUM_MAP[restore][0]; B.d[rk] = B.d._ymPlanLedger[rk]; } B.d._ymPlanAggregate = false; B.rec.set("data", B.d); app.save(B.rec); }
    return s;
  }
  if (!B.d._ymPlanLedger) { B.d._ymPlanLedger = {}; for (var base = 0; base < SUM_MAP.length; base++) { var bk = SUM_MAP[base][0]; B.d._ymPlanLedger[bk] = nz(B.d[bk]); } ch = true; }
  if (!B.d._ymPlanAggregate) { B.d._ymPlanAggregate = true; ch = true; }
...
function settle(app, col, body, who) {
  ...
  var d = JSON.parse(JSON.stringify(R.d)), map = [["실지출","전표실적"],["수입","표시_수입"],["판매수수료","판매수수료"],["유료관객","발권유료"],["무료관객","발권초대"],["문화나눔","문화나눔"],["오픈석","총오픈석"]], any = false;
  for (var i = 0; i < map.length; i++) { var input = nz(body[map[i][0]]); if (body[map[i][0]] === undefined || input === "") continue; var clean = input.replace(/,/g, ""); if (!/^-?\d+(?:\.\d+)?$/.test(clean) || !isFinite(Number(clean)) || Math.abs(Number(clean)) > 9007199254740991 || (i >= 3 && (Number(clean) < 0 || Math.floor(Number(clean)) !== Number(clean)))) return { error: map[i][0] + " 숫자 형식이 올바르지 않습니다", status: 400 }; d[map[i][1]] = String(Number(clean)); any = true; }
  for (var required = 0; required < 5; required++) if (nz(d[map[required][1]]) === "") return { error: map[required][0] + " 정산값이 필요합니다(실제 0이면 0 입력)", status: 400 };
  ...
  require(__hooks+"/ym-ticketlink-lib.js").refreshRevenue(app,col);
  touchLastmod(app, col);
